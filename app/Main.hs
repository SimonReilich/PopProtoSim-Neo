...
import Text.Colour
import Util
import Data.Text hiding (length, head, replace, replicate, find, any, null, map, concat, concatMap)
...
main = do
  ...
  when (args `isPresent` command "cut-stat") $ do
    ...
    mapM (\(x0, t) -> do
      ...
      ) (concatMap (\x0 -> map (x0, ) [(read tMin) .. (read tMax)]) [2 .. read x0Max]) >>= print
  ...
  when (args `isPresent` command "mod-stat") $ do
    ...
    mapM (\(x0, m) -> do
      ...
      ) (concatMap (\x0 -> map (x0, ) [(read mMin) .. (read mMax)]) [2 .. read x0Max]) >>= print
  ...
  when (args `isPresent` command "cmb-stat") $ do
    ...
    mapM (\(x0, m, t) -> do
      ...
      ) (concatMap (\(x0, m) -> map (x0, m, ) [(read tMin) .. (read tMax)]) (concatMap (\x0 -> map (x0, ) [(read mMin) .. (read mMax)]) [2 .. read x0Max])) >>= print
