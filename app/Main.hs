...
simulate :: Eq a => Eq b => Show b => Protocol a b -> [Int] -> Sniper a s -> Int -> Int -> Bool -> IO ()
simulate (m, d, s, o, t) x sn seed delay doCheck =
  let helper states sniper snipes gen =
        if isStable doCheck states (m, d, s, o, t)
          then
            ...
            (newStates, newSniper, snipeAmount, newGen) <- step states (m, d, s, o, t) sniper gen delay doCheck
            helper newStates newSniper (snipes + snipeAmount) newGen
   in putStrLn "" >> helper (Protocols.getInitial (m, d, s, o, t) x) sn 0 (mkStdGen seed)

step :: Eq a => Eq b => Show b => Configuration a -> Protocol a b -> Sniper a s -> StdGen -> Int -> Bool -> IO (Configuration a, Sniper a s, Int, StdGen)
step states (mapping, delta, stringify, output, test) (sniperState, sniping) gen delay doCheck = do
           ...
           in do
            ...
            if not (isStable doCheck newStates (mapping, delta, stringify, output, test))
              then do
                ...
                case killed of
                  Just i -> if (case newStates !! i of (b, _) -> b) 
